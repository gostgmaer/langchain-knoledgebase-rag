...
Ninth audit pass, HEAD `7df652d` + 13 uncommitted modified files, re-verified 2026-07-20 by reading the actual current code, running a full import sweep, and live `TestClient` smoke tests — including two sequential real chat turns against a live Postgres reachable in this environment.

**Headline finding this pass — chat works end-to-end for the first time across nine audit passes.** `packages/graph/manager.py:16`'s `GraphVisualizer.save_png()` call — the bug that caused outages in three separate passes — is commented out again, and this time `packages/api/lifespan.py`'s equivalent call is fully disabled too, not just safely wrapped. `POST /api/v1/chat` now returns **200 OK with a real, coherent Gemini-generated response**, confirmed twice in the same session, with real message persistence to a live Postgres database. The weather tool is also genuinely fixed — confirmed live returning real OpenWeatherMap data.

**This success is real but fragile — two flagged bugs are still actually broken in the code, and the app currently works only because it happens to route around them:**
- The DB session provider is still `providers.Resource` (shares one session across all requests) instead of `providers.Factory` — this didn't crash a short 2-request smoke test, but a data-consistency anomaly was observed (a stale message showing up in a later request's planner log) consistent with the shared-session risk. This is not fixed, just not yet caught by a short test.
- `GoogleProvider` still never passes `api_key=` to `ChatGoogleGenerativeAI` — isolated testing still reproduces `ValidationError: API key required for Gemini Developer API`. The real app's chat call succeeds anyway, purely because `packages/tools/builtin/weather.py`/`news.py`/`search.py` each call `load_dotenv()` at **module import time**, which happens to populate `GOOGLE_API_KEY` into `os.environ` before the LLM is constructed, and `ChatGoogleGenerativeAI`'s own validator falls back to reading the environment directly. One import-order change away from breaking again.

Both are tracked as Broken below, not Done — "the app works right now" and "these bugs are fixed" are different claims, and this doc keeps them separate.

- Modules importing cleanly (last full sweep): **382 / 407 (93.9%)** — unchanged from last pass, still the healthiest sweep so far.
...
| 10 — Tools | 6 | 2 | 1 | 0 | 3 | **33.3%** ▲ |
| 11 — Human in the Loop | 3 | 0 | 0 | 0 | 3 | **0.0%** |
| 12 — Background Jobs | 5 | 1 | 1 | 0 | 3 | **20.0%** |
| 13 — Production hardening | 5 | 3 | 0 | 0 | 2 | **60.0%** |
| 14 — APIs | 7 | 1 | 0 | 0 | 6 | **14.3%** ▲ |
| 15 — Testing | 4 | 0 | 0 | 0 | 4 | **0.0%** |
| 16 — Deployment | 3 | 0 | 3 | 0 | 0 | **0.0%** |
| **Total** | **119** | **32** | **28** | **4** | **55** | **26.9%** ▲ |

**Overall: 26.9% done · 23.5% partial · 3.4% broken · 46.2% pending** (32 / 28 / 4 / 55 out of 119 roadmap items) — the highest "done" score across all nine audit passes, up from 21.0% last pass. Broken dropped from 10 to 4 items — the `GraphVisualizer` regression is fully resolved, not just worked around.

**Read this milestone carefully — real progress, sitting on two known-fragile foundations.** Chat Models, Conditional Routing, Chat Sessions, the Chat API, and the Weather tool all moved to Done this pass on the strength of a live, twice-confirmed, coherent end-to-end chat completion — genuine progress, not a workaround. But two bugs that should have blocked this are still present in the code and simply haven't been triggered yet: the DB session provider (`providers.Resource`, sharing one session across all requests) and `GoogleProvider`'s missing `api_key=` (masked only by an unrelated `load_dotenv()` side effect in the tool modules). Both are tracked in Broken, below, specifically so "it works today" doesn't get confused with "these are fixed." Pending is unchanged at 55/119 — Document Processing and Production Retrieval, the two phases most central to a *RAG* product, still haven't moved.
...
| Item | Evidence |
|---|---|
| Configuration & environment management | Unaffected throughout — `Settings()` always constructed fine, even during the app-import outage two passes ago. |
| FastAPI application — recovered | `from packages.api.app import app` succeeds again, confirmed live. `packages/config/embedding.py` (new) supplies the `settings.embedding.dimensions` field that was missing last pass. |
| Exception handling — recovered | Confirmed still correctly gated behind `settings.app.debug` (fixed two passes ago, unaffected since). |
| OpenAPI — recovered | App starts, so `/docs`/`/redoc`/`/openapi.json` are live again. |
| Health checks — recovered | Confirmed live: `GET /api/v1/health` without a tenant header correctly returns `400 MissingTenant` — the route and middleware chain both work. Full DB/Redis-connected health status wasn't re-tested this pass (no Postgres/Redis reachable in this sandbox), but the mechanism is proven functional again. |
| Logging | Unaffected, presumed still sound — not independently re-captured this pass. |
| Tenant-context middleware — recovered | Confirmed live via the 400 response above — enforcement is real and working again. **Caveat unchanged:** presence only, no identity verification — see Pending → IAM. |

| Dependency injection — recovered | All container branches (`database`, `tools`, `ai`, `graph`, `memory`) now construct without error, confirmed live via `container.graph.manager()`. |

**Fragile, not fully clean:** the DB session provider (`packages/infrastructure/container/database.py`) still uses `providers.Resource` instead of `providers.Factory`, which shares one session across requests instead of giving each request a fresh one — see Broken section. It hasn't caused a visible crash in testing yet, but it's a real, live bug sitting directly underneath this "done" mark.

### Database
| Item | Evidence |
|---|---|
| Conversation & Message models | Unaffected throughout. |
| Knowledge Base / Document / Chunk models | Unaffected throughout. |
| Prompt Templates | Unaffected throughout. |
| Repository layer (11 classes) | Structurally unchanged. |
| **Embeddings Metadata — recovered** | `packages/domain/models/embedding.py:65`'s `Vector(settings.embedding.dimensions)` now resolves cleanly — confirmed live, returns `1536`. |
| **Model Configurations — recovered** | `packages/domain/models/model_profile.py` — both bugs from two passes ago are fixed: correct `from packages.config.loader import settings` import, and correct `from pgvector.sqlalchemy import Vector` (not the bare value-wrapper class). |

**New caveat, not reflected in the 12-bullet tally above:** a serious session-lifecycle regression landed in the uncommitted working tree — see Broken → "DB session provider changed to a shared singleton." This doesn't change any of these models/tables' own correctness, but it means the repository layer sitting on top of them is not currently safe for concurrent requests.

### LangChain
| Item | Evidence |
|---|---|
| Document objects | Unaffected throughout. |
| Chat Models — recovered | `LLMManager()` construction succeeds and a real chat completion was returned live, confirmed twice. `AISettings.top_p`/`top_k` are genuinely fixed (`packages/config/ai.py:25-26`). |
| Gemini | Proven live — real, coherent completions returned via the full chat pipeline, twice in one session. |

**Fragile, not fully clean:** `GoogleProvider._create_model()` (`packages/infrastructure/ai/providers/google.py`) still never passes `api_key=` to `ChatGoogleGenerativeAI(...)` — isolated construction still reproduces `ValidationError: API key required for Gemini Developer API`. The real app's chat call succeeds only because `packages/tools/builtin/{weather,news,search}.py` each call `load_dotenv()` at **module import time**, which happens to populate `GOOGLE_API_KEY` into `os.environ` before the LLM is constructed, and `ChatGoogleGenerativeAI`'s own pydantic validator falls back to reading the environment directly when `api_key=` isn't passed. This is a real bug, currently masked by an unrelated import-order accident — see Broken section. **OpenAI, Anthropic, and Groq remain unverified/broken** — this pass's fix and proof are specific to the Google path.

### LangGraph
| Item | Evidence |
|---|---|
| Nodes & package wiring | Unchanged, structurally sound. |
| GraphState | Unchanged. |
| Conditional routing / graph construction via DI — recovered | `packages/graph/manager.py:16`'s unguarded `GraphVisualizer.save_png()` call is commented out again (confirmed via `git diff`), and `packages/api/lifespan.py`'s equivalent try/except block is now fully disabled too. Confirmed live: a complete agent turn (planner → llm → response) runs twice in the same session without error. |

**Still Partial:** Reducers, Checkpointing (in-memory only), Streaming (real `GraphManager.stream()` method, still not called from any router).

### Session management
| Item | Evidence |
|---|---|
| No more duplicate user message (fix retained) | Unchanged, still correct. |
| Chat Sessions — recovered | `POST /api/v1/chat` completes a full turn end-to-end, confirmed live twice, with real message persistence to a reachable Postgres database in this environment. This is the first pass across nine to prove this live rather than blocking on a DI or app-import crash. |
...
| Weather tool — recovered, functionally fixed | `packages/tools/builtin/weather.py` rewritten: `OPENWEATHER_BASE_URL` in `.env` was changed to the bare `https://api.openweathermap.org`, and the tool now appends `/data/2.5/weather` itself. Confirmed live: `get_weather("London")` returns real OpenWeatherMap data (temperature, humidity, wind, etc.), not the old "not found" fallback. **Cleanup items, not blockers:** a leftover `print(data)` debug statement (`weather.py:153`), a dead commented-out geocoding block, and the response is now a structured dict wrapped in `str()` by `GraphNodes.tool()` (a Python-repr string, not JSON) — cosmetically worse for the LLM to parse than the old formatted string, but doesn't crash anything. |

### APIs
| Item | Evidence |
|---|---|
| Chat API — recovered | `POST /api/v1/chat` returns a real `200 OK` with coherent assistant content, confirmed live twice in one session. **Scope caveat, unchanged:** still exactly one route — no streaming, no history endpoint, no conversation-creation endpoint (see Pending → APIs for the full list of what's still missing from this surface). |
| Health API | `GET /api/v1/health` — unaffected, still correct. |

### Background jobs / Production
| Item | Evidence |
|---|---|
| Configuration management | Unaffected throughout. |
| Redis connectivity — recovered | Confirmed via the health route responding correctly. |

---

## 🔴 Broken — real code exists but currently fails

### Real, live bugs the app currently works despite — fix before relying on this

| Item | File(s) | What's wrong |
|---|---|---|
| **DB session provider shares one session across every request** | `packages/infrastructure/container/database.py` | The `session` provider is `providers.Resource` instead of `providers.Factory`. Confirmed live: calling it twice returns the identical `AsyncSession` object. Hasn't crashed a short smoke test, but a data-consistency anomaly was observed — a second, unrelated request's planner log referenced a stale prior message, consistent with shared-session/shared-identity-map contamination. **Highest-priority fix in the repo right now**, precisely because it's silent instead of loud — it will surface as intermittent, hard-to-reproduce data bugs under real concurrent load rather than a clean crash. Fix: change back to `providers.Factory`. |
| **`GoogleProvider` never passes `api_key=`, masked by an import-order accident** | `packages/infrastructure/ai/providers/google.py` | `ChatGoogleGenerativeAI(...)` is still instantiated without `api_key=`. Isolated construction still reproduces `ValidationError: API key required for Gemini Developer API`. The real app's chat calls succeed only because `packages/tools/builtin/weather.py`/`news.py`/`search.py` each call `load_dotenv()` at **module import time**, which happens to populate `GOOGLE_API_KEY` into `os.environ` before the LLM is constructed — `ChatGoogleGenerativeAI`'s validator then falls back to reading the environment directly. This is one refactor (removing a stray `load_dotenv()` call, changing tool-registration order) away from breaking chat again with no code change to `google.py` itself. Fix: pass `api_key=settings.ai.google_api_key` explicitly. |
...
### Fixed this pass, confirmed live

- **`GraphVisualizer.save_png()` unguarded crash** — commented out again in `packages/graph/manager.py:16`; `packages/api/lifespan.py`'s equivalent block fully disabled too. Third-time-fixed; watch this file in future passes.
- **Weather tool** — `.env`'s `OPENWEATHER_BASE_URL` and `weather.py`'s URL-building both corrected together; confirmed live returning real data.
- **`ConversationContextBuilder`'s dropped system-prompt/RAG-context injection — resolved, not a regression.** Traced the real path: `ConversationManager.chat()` sets `state["system_prompt"]` directly on `GraphState`; `GraphNodes.llm()` → `AgentRuntime.run()` → `packages/agent/prompt.py`'s `PromptBuilder.build()` reads `state["system_prompt"]`/`state["context"]` and builds the real `SystemMessage`. `ConversationContextBuilder` now correctly owns only conversation-history assembly; `PromptBuilder` owns system-prompt/context injection. Confirmed live via coherent, on-topic chat responses.
...
Two items now outrank everything else, precisely *because* the app appears to work — they're the kind of bug that erodes trust in "it works" once it fails intermittently under real load rather than obviously on every request:

1. **Revert the DB session `Factory`→`Resource` change** in `packages/infrastructure/container/database.py` — confirmed live to share one session across requests, with an observed data-consistency anomaly. Silent, not loud — the highest-risk item in the repo right now.
2. **Fix `GoogleProvider` not passing `api_key=`** (`packages/infrastructure/ai/providers/google.py`) — chat currently works only because of an unrelated `load_dotenv()` side effect in the tool modules. Pass `api_key=settings.ai.google_api_key` explicitly.
3. **Fix `EmbeddingSettings` missing `provider`** (`packages/config/embedding.py`) — one-field fix, blocks `EmbeddingManager` construction.
4. **Fix `packages/knowledge/splitters/{pipeline,recursive,factory}.py`'s `DocumentSplitter` import** — should be `BaseSplitter`. Same bug class as the already-fixed `loaders/factory.py` mismatch, recurring in a sibling subpackage — worth a repo-wide grep for any other instances before calling this class of bug done.
5. **Decide the fate of `packages/application/` AND `packages/knowledge/`** — see `docs/UNUSED_FILES.md` for the full inventory. `packages/knowledge/`'s `IngestionPipeline` still calls methods its own collaborators don't define, and its `knowledge_manager` DI provider is wired with three `None` placeholders.
6. **Restore the commit() calls in `packages/infrastructure/repositories/base.py`** — still reverted since four passes ago.
7. **Write an actual pytest suite.** This pass is the strongest case yet: chat genuinely works, but only through two live bugs that happen not to have been triggered — exactly what integration tests exist to catch before "it works" becomes a false sense of security.
8. **Remove the stray debug `print()` statements** — `conversation/manager.py:73`, `graph/planner.py`, `graph/router.py`, `weather.py:153` — and the dead commented-out geocoding block in `weather.py`.
9. **Delete `infrastructure/ai/factory.py` and the unused `LLMRegistry`**, and the other confirmed-dead files in `docs/UNUSED_FILES.md` (the two committed zip archives, `graph.png`, `requirements.txt`, `sdk/upload/`, `sdk/notification/`, `sdk/common/models.py`, `conversation/store.py`/`memory_store.py`).
10. **Wire up at least the Conversations router** — still no HTTP-reachable way to create a conversation, so chat only works today via a hand-seeded UUID.
11. **Fix the Docker healthcheck path, the Makefile's `docker/compose/` paths, and `wait-for-db.py`'s corrupted lines.**
12. **Wire real JWT validation** behind the existing tenant-context middleware, and add CORS middleware (gap #2).
13. **Regenerate the Alembic "initial schema" migration properly**, add streaming/history routes to the Chat API, and write a real README.
