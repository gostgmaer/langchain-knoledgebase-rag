...
**Still Partial, unaffected:** Reducers, Streaming.
...
**Practical impact:** a conversation can now genuinely be created over HTTP and chatted with end-to-end — verified live with real Gemini responses, real persistence, real tool-calling, and real cross-conversation memory recall. What's still missing on the routing surface is just breadth, not defects: no history/listing route, no delete, no streaming route (`GraphManager.stream()` exists but nothing in `chat.py` calls it).
