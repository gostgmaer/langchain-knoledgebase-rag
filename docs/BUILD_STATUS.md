...
| **Total** | **119** | **60** | **15** | **0** | **44** | **50.4%** ▲▲▲▲▲▲▲▲▲▲ |

**New all-time high — 50.4%, up from 42.0%. This project crosses the halfway mark for the first time. Decision, by explicit request: `packages/knowledge/` — not `packages/rag/` — is now the canonical document-processing/RAG implementation, live-wired into the chat graph.**
...
7. **Fix the Docker healthcheck path and Makefile paths, wire real JWT validation, and regenerate the Alembic "initial schema" migration properly** — the migration still has no `create_table` calls, and now also doesn't know about the `model_profiles.vector`/`memories` table schema added via direct `ALTER TABLE`/`create_all` this session.
