...
| 9 — Production Retrieval ⭐ | 13 | 3 | 1 | 0 | 9 | **23.1%** ▲▲▲ |
...
| **Total** | **119** | **50** | **23** | **0** | **46** | **42.0%** ▲▲▲▲▲▲▲▲▲ |

**New all-time high — 42.0%, up from 40.3%. Asked directly why Reducers and Streaming were still Partial/Pending — both are now genuinely Done, closing out Phase 5 (LangGraph) at 66.7%, up from 44.4%, with zero Partial items left.**
...
| Item | Status |
|---|---|
| **Query Embedding — Done, first time ever** | A real chat message routed to `retrieve` now reaches Chroma's embedding function live, confirmed via a genuine `200 OK` end-to-end chat response, not just a unit-level call. |
| **Dense Retrieval / Similarity Search — Done, first time ever** | `RetrieveNode` now calls `RetrievalPipeline.retrieve()` → `VectorStoreManager.similarity_search()` — the same Chroma-backed stack this session's memory work already proved live — instead of the broken, never-wired `KnowledgeManager` facade. |
...
| Context building, prompt construction for retrieval | Partial: `PromptBuilder` now assembles retrieved chunks into a real prompt block, but with no deduplication or token budget |
| Citation generation | `message_citation.py` DB model exists but nothing populates it |

Still the phase furthest from its own ambition — classification, rewriting, expansion, hybrid/BM25, re-ranking, and citations remain entirely unimplemented. But the foundational retrieval call is no longer just unreachable, it's proven: `RetrieveNode` was previously wired to `KnowledgeManager`, whose DI provider (`packages/infrastructure/container/rag.py`) stubbed all three of its collaborators as `providers.Object(None)`, and whose own `search()` method has an internal bug (misusing the file-ingestion `IngestionRequest` schema as a search-request wrapper). Rewired to the already-working `RetrievalPipeline`/`VectorStoreManager` stack instead of repairing `KnowledgeManager` — confirmed live via a real chat request and a full 3-scenario regression (chat, cross-conversation recall, tool-calling) afterward. `packages/knowledge/`'s parallel implementation remains unwired and internally broken, exactly as noted above; `KnowledgeManager` is now provably dead code, not a hidden production path.
...
**Fixed this session:** the DB session `Resource`→`Factory` bug and the connection-leak/multi-session risks it could have introduced; the `model_profiles.vector` schema drift; the repository-writes-never-commit bug; the conversations router (stub → real, working, idempotent `POST /conversations`); replacing `ConversationManager`'s flow with `packages/application/services/chat_service.py`'s `ChatService`, 9 further real bugs across that stack — resulting in the first fully working `POST /api/v1/chat` round trip this project has ever had; real tool-calling (`LLMNode` was never binding tools to the model at all); real long-term memory — a genuine `memories` table, real pgvector search, and a significant `Singleton`-caching-a-pre-request-session bug fixed along the way; and, most recently, real `ChatPromptTemplate`/LCEL/`BaseOutputParser` usage across `PromptBuilder` and the memory extraction/summarization chains, surfacing one more real bug (`LLMManager` isn't a `Runnable`, fixed via `.model`). See the milestone section at the top of this document for the full account of each. **Fixed in earlier passes, holding steady:** `SafeCalculator`'s `ast.Num` and `dataclass(slots=True)`/`__dict__` crashes.
