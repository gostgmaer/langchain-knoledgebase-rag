...
| Persistent Sessions | Checkpointer is `MemorySaver` — now a real, correctly thread-scoped, process-lifetime Singleton (see Checkpointing above), but still in-memory only — no conversation state survives a process restart |
...
| Hybrid / BM25 search, re-ranking | Not implemented — `packages/knowledge/retrievers/providers/hybrid.py` correctly raises `NotImplementedError`; `mmr.py`/re-ranking equivalents are the same |
| Context building, prompt construction for retrieval | Partial: `PromptBuilder` assembles retrieved chunks into a real prompt block, but with no deduplication or token budget |
...
| Document Search tool | Empty stub |
...
7. **Fix the Docker healthcheck path and Makefile paths, and regenerate the Alembic "initial schema" migration properly** — the migration still has no `create_table` calls, and now also doesn't know about the `model_profiles.vector`/`memories` table schema added via direct `ALTER TABLE`/`create_all` this session. ~~Wire real JWT validation~~ — **Done** (via IAM delegation, not local signature verification — see IAM section above). Refresh Token Validation remains genuinely open.
