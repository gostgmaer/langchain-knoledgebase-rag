...
| **Total** | **119** | **36** | **34** | **0** | **49** | **30.3%** ▲▲▲ |

**New all-time high — 30.3%, up from 26.9%.** Memory is the story of this update: 4 of its 5 items moved from Partial to Done (Semantic Memory, User Preferences, User Facts, plus Conversation Memory already counted) once the pipeline went from "computes and discards" to "genuinely persists and retrieves." Episodic Memory stays Partial — the infrastructure could support it, but nothing in the current extraction categories (`fact`/`preference`/`profile`/`task`/`summary`/`goal`/`skill`/`project`) specifically captures discrete past-event memories distinct from general facts.
...
| **Episodic Memory — still Partial** | The storage/retrieval infrastructure could support it, but the current extraction categories (`fact`/`preference`/`profile`/`task`/`summary`/`goal`/`skill`/`project`) don't distinctly capture discrete past-event memories separate from general facts — not implemented as its own concept yet. |
...
8. **Implement Episodic Memory** — the last unfinished item in Phase 7. The storage/retrieval infrastructure is real now; needs a distinct extraction category (and prompt guidance) for discrete past-event memories, separate from general facts/preferences.
