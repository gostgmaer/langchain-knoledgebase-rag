...
This roadmap describes the *target* — what's in scope and when. It intentionally says nothing about what's actually built yet. That's tracked separately and kept current in [`docs/BUILD_STATUS.md`](./BUILD_STATUS.md), which is re-verified against the running code (not just read from source) on every audit pass: what genuinely works end to end, what's a partial/unverified implementation, what's written but currently broken, and what hasn't been started. As of the latest pass, the biggest gap between this roadmap and reality is concentrated in Phase 2 (IAM — tenant context exists, but there's no real JWT/RBAC enforcement yet), Phase 9 (Production Retrieval — the starred centerpiece is the least-built phase relative to its own ambition), Phase 14 (APIs — only Chat and Health are real, and Chat itself is only a single non-streaming endpoint so far), Phase 15 (Testing — no real assertion-based test suite exists yet), and Phase 16 (Deployment — no Dockerfile, an empty `docker-compose.yml`).
