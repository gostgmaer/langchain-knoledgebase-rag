...
🚀 Version 2.0 — analyzed against the real, current codebase (not just described) after v1.2 wrapped; 5 of the 15 concrete items (Knowledge Graph/Graph RAG and Multi-Agent excluded, both needing an upfront decision this roadmap doesn't make) are now built and live-verified. Full account of what was checked, built, and verified in [`docs/BUILD_STATUS.md`](./BUILD_STATUS.md).
...
- ❌ **Knowledge Graph** — genuinely from scratch: no entity/relationship extraction pipeline, no graph storage layer exists anywhere in this codebase. Needs a real architecture decision first (dedicated graph DB vs. modeling edges/nodes in Postgres, matching this project's established "no new infra unless the existing stack genuinely can't do it" pattern) before any code gets written.
- ❌ **Graph RAG** — strictly depends on Knowledge Graph existing first; not independently startable.
...
- ❌ **Subgraphs** — the graph is still one monolithic `StateGraph` (6 nodes, no sub-compilation). No concrete pain point driving decomposition yet — a composability refactor waiting for a reason, not attempted this pass.
...
### Multi-Agent — confirmed zero existing scaffolding, needs a real use case before it needs an estimate
`packages/agent/` was fully deleted in a commit predating this session (`de5c574`, "Remove deprecated agent and RAG components"), and there is no supervisor/delegate/sub-agent concept anywhere in the current codebase — this category is a genuine from-scratch architecture project, not an extension of anything that exists. Nothing in this project's extensive live testing (this session or earlier) has surfaced a concrete case where the current single-agent-with-tools-and-retrieval design is actually falling short — before scoping implementation effort here, the real open question is *what specific task the current design can't handle*, not engineering estimates for the 6 items below.
