> **Note on symbols**: ✅ = in scope for the named version (**not** "already built"), ⏸ = deferred/paused for a later version, ❌ = explicitly out of scope for now, 🟡 = out of scope for now but already substantially satisfied by existing work in an earlier version (used in the Version 2.0 section after a real codebase audit — see its own note). This file is the **target roadmap** — what we've agreed to build and in which version. For what is *actually* built, working, broken, or missing right now, see [`docs/BUILD_STATUS.md`](./BUILD_STATUS.md), which is re-verified against the live code on every audit pass.
...
🚀 Version 2.0 — **all 15 concrete items now built and live-verified**, closing this roadmap. The 5 Advanced Retrieval/LangGraph items landed first; Knowledge Graph, Graph RAG, Subgraphs, and Multi-Agent (all previously ❌, each blocked on a decision only the roadmap owner could make) landed in a second pass once those decisions were made: Knowledge Graph storage → Postgres tables, not a dedicated graph DB; Multi-Agent's driving use case → multi-part comparison/synthesis questions coming out shallow; Subgraphs → satisfied by a new subgraph purpose-built to serve Multi-Agent's Researcher role, not a speculative refactor of the existing single-question graph. Full account of what was checked, built, and verified in [`docs/BUILD_STATUS.md`](./BUILD_STATUS.md).

### Advanced Retrieval — built in cost order, cheapest first
- ✅ **Self Query Retriever** — `packages/knowledge/retrievers/providers/self_query.py`, a new `"self_query"` strategy composing over hybrid search. Reuses `LLMManager.with_structured_output()` (the same pattern `QueryAnalyzer` uses) to extract `section`/`page_number` filters from natural language, merged into `SearchFilter.metadata` — already wired end-to-end into Chroma's `where` clause, no schema change needed for the filtering itself. A real casing bug was found and fixed live: the LLM extracts natural phrasing ("specifications") while the real stored heading keeps its original case ("Specifications"); fixed with a `section_lower` shadow field for exact-match comparison.
- ✅ **Parent Document Retriever** — `packages/knowledge/retrievers/providers/parent_document.py`. Small-chunk match for precision, then reassembled from same-`(document_id, section)` sibling chunks via the already-existing `list_chunks()` — no separately stored parent blob, no reopening Document Processing (Phase 8)'s "chunk content stays vector-store-only" decision.
- ✅ **Multi Vector Retriever** — `packages/knowledge/retrievers/providers/multi_vector.py`, v1 scope: one extra document-level summary representation per document (not per-chunk Q&A pairs, a deliberate cost-bounding cut), generated at ingestion time and resolved back to real primary chunk content before ever reaching the LLM/citations — the synthetic summary text itself is never returned as literal context. Two real bugs found and fixed live while building this: Gemini's list-shaped message content was being stringified literally into the summary text (fixed by reusing the existing `normalize_message_content` utility), and a summary hit could resolve to a chunk the inner retriever had *also* matched directly, producing a literal duplicate result (fixed with an id-keyed dedup pass).
- ✅ **Knowledge Graph** — modeled in Postgres (`entities`/`relationships`/`entity_mentions` tables, `packages/domain/models/entity.py`/`relationship.py`/`entity_mention.py`), tenant-wide rather than per-knowledge-base since `SearchFilter` — the only thing the retriever has to query with — has no `knowledge_base_id` field. A new `GraphExtractor` (`packages/knowledge/extraction/graph_extractor.py`) runs one structured-output LLM call per document (same cost-bounding precedent as Multi Vector Retriever's summary — not per chunk), fails open on any error. Entity `entity_type`/relationship `relationship_type` are deliberately plain `String` columns, not `sqlalchemy.Enum` — LLM-extracted values are open-ended, and a native Postgres enum here would reproduce the exact drift bug (`MemoryType`, hand-run `ALTER TYPE ... ADD VALUE`) already documented elsewhere in this project. Verified live: two real documents ingested, the LLM correctly extracted `Ferrolabs Robotics -[manufactures]-> FLX-77Q2`, `Ferrolabs Robotics -[partners_with]-> Acme Logistics`, `Acme Logistics -[deploys]-> FLX-77Q2`, `Acme Logistics -[operates_in]-> northeast United States` — and `Acme Logistics`, mentioned in both documents, was correctly deduplicated into one entity row rather than created twice, proving `EntityRepository.get_or_create()`'s dedup works for real, not just in a unit test.
- ✅ **Graph RAG** — `GraphRAGRetriever` (`packages/knowledge/retrievers/providers/graph_rag.py`), a new `"graph_rag"` strategy composing over `HybridRetriever` like every other Advanced Retrieval item. Matches the query against known entity names via cheap substring matching (not another LLM call — the same cost trade-off Self Query Retriever already made), does a 1-hop relationship traversal from any matches, and pulls a few real top-scoring chunks from newly-reached documents via a scoped `similarity_search()` — mirroring Multi Vector Retriever's "resolve a signal back to real chunk content" pattern; the graph traversal itself never becomes literal context. Required a real DI change, not incidental: `RAGContainer.retriever`/`retriever_manager` moved from `Singleton` to `Factory`, since a graph-aware retriever needs per-request repository access — the same DI-lifetime rule (`Singleton` + a per-request session = a permanently frozen first-request session) this project has already been bitten by twice. Verified live with a direct, honest test: a deliberately narrow inner retriever (top-1 only, filtered to chunks literally mentioning "Ferrolabs" — simulating a large-corpus scenario where the Acme-only chunk wouldn't otherwise rank) found only the Ferrolabs chunk on its own; wrapped in `GraphRAGRetriever`, the same query correctly traversed the `partners_with` edge and pulled in Acme's own "Founded in 2015" chunk — the literal chunk needed to answer a question the narrow search alone couldn't.

### Advanced LangGraph — 3 of 4 done or already substantially satisfied
- ✅ **Parallel Execution** — `packages/graph/builder.py` already fanned `planner`/`load_memory` out concurrently; the one remaining concrete gap, `RetrieveNode`'s multi-query loop (`packages/graph/nodes/retrieve.py`), is now `asyncio.gather`-based instead of sequential.
- 🟡 **Dynamic Routing** — already substantially satisfied by existing work, not touched further this pass. `GraphPlanner` (`packages/planner/planner.py`) already makes a real LLM call (`QueryAnalyzer.analyze()`) to decide `needs_retrieval` per turn — genuinely "routing decisions computed at runtime from more sophisticated policies," the roadmap's own definition, not a static conditional edge.
- ✅ **Subgraphs** — reframed honestly rather than forced: the original 6-node graph still has no motivating pain point for decomposition (that's exactly why it stayed ❌ through the first Version 2.0 pass). Satisfied instead by a new, small, reusable compiled sub-`StateGraph` (`packages/graph/subgraphs/research.py`, `ResearchSubgraphBuilder` — a minimal `retrieve → synthesize → END` graph over its own `ResearchState`, not `GraphState`) purpose-built to serve Multi-Agent's Researcher role below — genuinely load-bearing, not a speculative refactor. Invoked via a plain `.ainvoke()` call from inside an ordinary node (`ResearcherNode`), not LangGraph's subgraph-as-node primitive — subgraph-as-node only supports one static invocation slot and can't fan out a runtime-determined number of parallel sub-question investigations, which is exactly what's needed here. Verified standalone (direct `.ainvoke()` against real ingested content, no HTTP) before ever being wired into the parent graph.
- ✅ **Durable Execution** — a new `ConversationStatus.PROCESSING` state + `processing_started_at` timestamp (`packages/domain/models/conversation.py`), set/committed by `ChatService` right before every `invoke()`/`resume()`/`stream()` call and cleared on any normal return (a legitimate tool-approval pause included — only a real process death mid-call skips the clear). A new `recover_stuck_conversations_job` (every ~5 minutes, `packages/worker/jobs.py`) finds conversations stuck `PROCESSING` past a 5-minute threshold and re-drives them via a bare `graph.ainvoke(None, ...)` continuation, persisting the completed turn through the same finalize path a normal turn uses. Verified live end to end: a real turn's mark/clear lifecycle, a manufactured stuck-and-recoverable conversation actually completing, and a legitimately fresh in-flight conversation correctly left alone. A real duplicate-message bug was found and fixed live in the process: recovering an *already-fully-completed* thread (a narrow crash-timing edge case) doesn't error, it just re-returns the final state — now guarded by a `GraphManager.has_pending_work()` check before re-invoking, confirmed live to have actually produced a duplicate message before the guard existed.

### Multi-Agent — built around a real, chosen driving gap, not a speculative template
`packages/agent/` was fully deleted in a commit predating this session (`de5c574`, "Remove deprecated agent and RAG components") — this was genuine from-scratch architecture work, not an extension of anything that existed. Rather than build all 6 originally-listed sub-roles speculatively, the roadmap owner picked one concrete, confirmed gap to build around: **multi-part comparison/synthesis questions come out shallow** — the pre-existing pipeline ran one parallel multi-query retrieve pass and one LLM pass, never decomposing a multi-part question into independent sub-investigations, so answers blended sources or missed angles. Three roles were built to close exactly that gap; two were deliberately left out.
- ✅ **Supervisor** (`packages/graph/nodes/supervisor.py`) — decides whether a turn's question is genuinely multi-part via one `with_structured_output` call, and if so, decomposes it into 2-5 sub-questions. Also serves the **Planner** role from the original 6-item list — a separate Planner sub-agent wasn't built because decomposition-into-an-execution-plan and the multi-part/not decision are the same judgment call here, not two. **Hard fail-open guarantee, not just a try/except**: an LLM error, *or* a response nonsensically claiming multi-part with zero real sub-questions, both collapse to `is_multi_part=False` — routing falls straight through to the original, byte-identical `retrieve → llm` path. Verified live, both branches: a genuine single-entity question ("what products does Ferrolabs Robotics make?") routed `join → supervisor → retrieve → llm`, the exact unchanged topology; the real comparison question routed `join → supervisor → researcher → writer`.
- ✅ **Researcher** (`packages/graph/nodes/researcher.py`) — runs the new research subgraph (Subgraphs, above) once per sub-question via `asyncio.gather`, the same parallelization idiom `RetrieveNode`'s own multi-query fan-out already uses.
- ✅ **Writer** (`packages/graph/nodes/writer.py`) — synthesizes every sub-question's finding into one coherent answer addressing each distinctly, via the same inner `ChatService` `LLMNode` uses, no tools bound. Output satisfies the exact `messages`/`usage`/`citations` contract `LLMNode` already guarantees — `ChatService._finalize_or_pause` needed zero changes. Verified live: asked to compare Ferrolabs Robotics and Acme Logistics, the response came back with a distinct section per company plus a synthesized "Nature of the Partnership" section, correctly grounded in both documents' real content.
- ❌ **Reviewer** / ❌ **Citation Validator** — deliberately not built this pass. Neither was the confirmed gap driving this work ("citations aren't verified" was an option considered and explicitly not chosen), and building them anyway would have repeated the exact "generic template, not a real need" anti-pattern this scoping decision was made to avoid. Left as a natural future extension if that specific gap is separately confirmed as real.

New `GraphState` fields: `is_multi_part`, `sub_questions`, `research_findings` (plain overwrite, no reducer — single write per turn). A new `ResearchFinding` dataclass (`packages/graph/schemas.py`) is reachable from `GraphState` on every multi-part turn and was added to the Postgres checkpointer's `_CHECKPOINT_SERDE` allowlist (`packages/infrastructure/container/graph.py`) — this project has shipped a no-op fix here once before by missing exactly this step.
...
**Web Search, Weather, and News proven live, closing out the last flagged gap from the priorities list: "Calculator's live invocation is proven, these three aren't individually yet."** No code was broken — the real blocker (tool binding) was already fixed; these three simply hadn't been individually exercised through the chat pipeline. All three API keys were already configured. Live-tested each with a fresh conversation: Weather made a genuine `GET https://api.openweathermap.org/...` call and returned real London conditions (22.44°C, clear, 48% humidity); Web Search (`GoogleSerperAPIWrapper`) returned a specific, current, correctly-dated real-world result; News (Tavily-backed) returned multiple specific current headlines with real named companies. Also cleaned up two stale leftovers found along the way in `docs/BUILD_STATUS.md`: a Phase 10 "Pending" line still claiming "Calculator tool | Empty stub" despite the Done section right above it saying otherwise, and an old note describing the Weather tool as "downgraded to Partial... live proof currently unobtainable," several passes out of date. Tools (Phase 10) moves from 16.7% to 66.7% — only Knowledge Base Search and Document Search remain unimplemented. Score: 38.7%, a new all-time high.

**Asked to check on the LangGraph phase's partial items, Checkpointing turned out to be a real bug — and its investigation revealed an earlier fix this session had never actually worked.** `GraphManager` was always correctly passing `thread_id` into the checkpointer's config on every call; the real problem was that `GraphBuilder` constructed a brand-new, empty `MemorySaver()` on every single request (both `builder` and `manager` are DI-wired as `providers.Factory`), so LangGraph's own checkpointing provided zero benefit across turns — conversation continuity only ever worked because message history gets reloaded from the database each turn, independent of the checkpointer. Fixed by wiring the checkpointer as its own `providers.Singleton`, injected into `GraphBuilder` — safe here (unlike the earlier memory-Singleton bug) since `MemorySaver` holds no per-request database session. Making checkpointing genuinely persistent then exposed that the msgpack-allowlist fix from earlier this session had been a complete no-op the whole time: `JsonPlusSerializer().with_msgpack_allowlist(...)` silently does nothing whenever the base serializer starts in its default permissive mode, which it always does — so the original "zero warnings across two turns" verification was a false negative, since no checkpoint had ever actually been reused to trigger real deserialization. Corrected by passing the allowlist at construction instead, and — since it's now genuinely restrictive — auditing every custom type reachable from `GraphState` and adding the ones that were missing (`SearchResult`, `Citation`, `MemoryFact`, `MemoryType`), which had started hard-blocking rather than just warning the moment the fix became real. Verified live with a 3-turn same-conversation test (remember a number, recall it, do math on it): zero warnings, zero blocked deserializations, correct behavior throughout. Checkpointing and Thread IDs both move from Partial to Done. Score: 40.3%, a new all-time high.

**Asked directly why Reducers and Streaming were still Partial/Pending — both are now genuinely Done, taking Phase 5 (LangGraph) to zero Partial items for the first time.** Reducers: `GraphState` had exactly one field with a real reducer (`messages`, via the built-in `add_messages`); every other field just overwrote on each write. Found a genuine case that actually needed accumulation: `usage` (LLM token counts) was never even populated, despite the underlying `ChatResponse.usage` already carrying real data on every call — and a single turn can invoke the LLM node twice (once before a tool call, once after), so overwriting would silently lose the first call's tokens. Added a real custom reducer, `merge_usage()`, that sums numeric fields while safely handling LangChain's non-numeric nested usage sub-fields. Streaming: `GraphManager.stream()` was real code nothing ever called — the `stream` request field was accepted and then hardcoded to `False` before reaching `GraphState`, and the router had no `StreamingResponse` path at all. Built genuinely across five files: `LLMNode` now streams token-by-token via `get_stream_writer()` when requested (fixing along the way a bug where `ChatService.astream()` bypassed tool binding entirely); `GraphManager.stream()` now uses `stream_mode="custom"` so those events actually surface; the application-layer `ChatService` gained a real `stream()` method persisting the fully-assembled message once the stream completes; the router now returns genuine SSE (`text/event-stream`) when `stream: true`. Verified live: a real tool-calling turn produced a correctly-summed `usage` dict; a real SSE request streamed multiple progressive token events before a `done` event, with the assembled text confirmed via direct database query to match exactly one correctly-persisted message — no duplication — and tool-calling plus cross-turn memory recall both still work over the streaming path. LangGraph (Phase 5) moves from 44.4% to 66.7%. Score: 42.0%, a new all-time high.

**Asked what was actually wrong with Document Processing, then — by explicit request mid-fix — `packages/knowledge/` became the canonical RAG implementation instead of `packages/rag/`, and this project crosses 50% for the first time.** The audit found `packages/rag/` worked for `.txt` but nothing else was verified, two of four documented loaders crashed on a missing `unstructured` dependency, HTML support didn't exist, and there was no tenant isolation. Mid-fix, the decision changed: `packages/knowledge/` — a second, ~70-file, more ambitious implementation that had sat alongside `packages/rag/` all session, documented as unwired and broken — should be the one true implementation. A first attempt went too far, deleting most of its files for a simplified rewrite; correctly stopped and cleanly reverted via `git checkout --` (nothing had been committed), then redone properly: fix every file in place, keep the existing structure.

What was actually wrong, and had never once successfully run: a stray `__init__` sitting at module scope in `loaders/base.py`, completely outside the class it belonged to; 5 of 7 loader subclasses requiring a constructor argument the factory never passed; a call to a nonexistent `handle_exception` method; a `DocumentSplitter`/`BaseSplitter` import mismatch already flagged in earlier passes as unfixed; a wrong embedding-provider config attribute, also already flagged — and a deeper one earlier passes missed, where the embedding *model name* was silently reading from the chat model's env var, causing real API calls to 404; a missing `embed_query()` needed for search to work at all; `SearchResult` constructor mismatches and 5 missing abstract methods in both vector-store backends; a systemic bug across 5 files misusing a file-ingestion schema as a search-request type; a duplicated, shadowed import in `KnowledgeManager` itself (the confirmed root cause of an earlier-documented crash); an `IngestionPipeline` that called three methods none of its real collaborators actually had; and a ChromaDB version-compatibility bug in the metadata filter, found only once search was finally reachable.

A genuine bonus, found by accident: tenant isolation, explicitly deferred as "later" work, turned out to already be designed into `packages/knowledge/`'s vectorstore — fixing the filter bug that blocked search from working at all also activated it. Verified live: the same search under two different tenants returned the right chunk for one and zero results for the other.

Wired for real into the live chat path: the DI container now builds the full loader → cleaner → splitter → embeddings → Chroma-vectorstore → `KnowledgeManager` chain from `packages/knowledge/`; `RetrieveNode` calls `KnowledgeManager.search()`; the memory pipeline's embedding dependency now points at the same package (a compatible drop-in). `packages/rag/` is untouched but no longer reachable from the live graph. Verified live, full pipeline: a real `.txt` document ingested through the DI-wired app, then a real chat request correctly retrieved and cited its exact content — not a paraphrase, the precise figure from the source. Full regression (normal chat, cross-turn memory, tool-calling) still passes.

Document Processing (Phase 8): 0.0% → 60.0%. Production Retrieval (Phase 9): 23.1% → 30.8%. **Score: 50.4% — past the halfway mark of this project's own roadmap for the first time.**

**Asked to implement IAM using the existing `packages/sdk/iam/` client SDK ("all endpoints are already included") — it had never once been exercised, and is now wired into a real authentication middleware with working RBAC.** `packages/sdk/iam/{auth,user,tennets}.py` all built their HTTP client with `base_url=str(settings.url)`, but the real `IAMSettings` field is `base_url` — confirmed live via `AttributeError` before the fix, the same "never actually run" pattern found in `packages/knowledge/` earlier this session. Fixed in all three files, plus a second dead-code bug found while wiring the shared HTTP client factory (`http2=True` with the `h2` package not installed).

Since no real IAM backend is reachable in this environment (`localhost:3000` refuses connections; `.env`'s IAM secrets are placeholder values), two decisions were made explicitly with the user before writing code: authenticate via a per-request call to `IAMClient.auth.get_current_user()` with the Bearer token, not local JWT/JWKS verification (uses only what's already coded, no new dependencies); and **fail open** — missing/invalid tokens or an unreachable IAM service fall back to the existing default-tenant/default-user behavior rather than rejecting, with real permission enforcement only activating once a token is actually verified. This kept the entire regression suite built all session (no auth headers) working unchanged.

Built: `AuthService` (the first real code in a package that had been an empty stub all session), a new `IAMContainer` (the first `packages/sdk/*` client ever wired into DI — Upload and Notification remain equally unwired), a real `AuthenticationMiddleware` using `dependency_injector`'s `@inject`/`Provide[...]` pattern inside a Starlette middleware for the first time (not just a FastAPI route dependency, where it had only been used before), and `require_permission()`/`require_role()` dependency factories. Existing routes weren't retrofitted with specific required permissions — no taxonomy exists for them yet; this delivers the reusable mechanism, not a guessed authorization policy.

Verified against real infrastructure, not mocks: `AuthService.resolve()` against the actual unreachable configured URL correctly logs and returns `None`; a live chat request with a bogus Bearer token produced the exact expected fail-open log line and still completed correctly end to end, including a real tool call; `require_permission()`/`require_role()` unit-verified on all four branches. IAM (Phase 2): 28.6% → 71.4% — User Context, RBAC, and Permission Validation move to Done; JWT Validation moves to Partial (real validation, via IAM delegation rather than local signatures); Refresh Token Validation remains the one open item. **Score: 52.9%, a new all-time high.**

**Asked to complete Document Processing — six remaining items done, and Phase 8 reaches 100%, the second full phase this project has ever had.** Two decisions made explicitly with the user before building, since the remaining work needed real architectural choices, not just fixes: async indexing uses FastAPI `BackgroundTasks` rather than the already-installed-but-fully-dead `arq` dependency (zero new infrastructure; real `arq` wiring is Phase 12 territory); and a real document persistence layer plus a `POST /api/v1/documents` route were built, since without them Incremental Indexing has no way to detect a repeat upload and none of this work would be provable through the running app rather than direct Python.

Building that persistence layer surfaced two more real, never-exercised repository bugs (the same "written, never run" pattern found in `packages/knowledge/` and `packages/sdk/iam/` earlier this session): `DocumentRepository.get_by_filename` filtered a nonexistent column (real one: `file_name`); `KnowledgeBaseRepository.list_enabled` filtered a nonexistent column (real one: `status`), and the existing `get_by_name` wasn't even tenant-scoped. Both fixed, and a new `packages/knowledge/bootstrap.py` auto-provisions a default `KnowledgeBase` per tenant, mirroring the existing conversation-bootstrap pattern — nothing had ever created one before.

Metadata extraction now pulls real `author`/`page` from PDF (already native) and DOCX (added via `python-docx` core properties, since the DOCX loader only extracted text before), real token counts via `tiktoken` instead of a hardcoded zero, and `ingested_at` timestamps. Markdown-aware chunking uses `langchain_text_splitters.MarkdownHeaderTextSplitter` (already installed) in a two-stage header-then-size split. Semantic chunking is hand-rolled — no `langchain_experimental` dependency — embedding every sentence in one real batch call and breaking chunks at the 95th-percentile cosine-distance jump; verified live on genuinely mixed-topic text (robotics then a cooking recipe), which split exactly at the topic boundary. Batch indexing was a real bug, not just inefficiency: one embedding API call per chunk in a loop, replaced with a single real batch call — verified via a call-count check (20 chunks, exactly 1 call). Incremental indexing computes a real SHA-256 checksum before any work and skips re-embedding unchanged re-uploads — verified live with two identical uploads, the second returning `skipped=True` and reusing the same document ID.

A DI-lifetime detail worth flagging given this session's history with exactly this bug class: `ingestion_pipeline` and `knowledge_manager` now depend on a request-scoped repository, so both were changed from `Singleton` to `Factory` — the same fix applied earlier to the memory pipeline; getting it wrong here would have silently orphaned every document status update into an uncommitted session.

Verified live, full pipeline, through the real running app: a real multipart upload returns `202 Accepted` immediately, the background task flips the document's status through to `READY`, and a real chat request afterward correctly retrieved and cited the uploaded content by exact figures. Full regression (memory recall, tool-calling, streaming) still passes. Document Processing (Phase 8): 60.0% → **100%**. The new Upload API also moves APIs (Phase 14) from 14.3% to 28.6%. **Score: 58.8%, a new all-time high.**
