...
As of the tenth audit pass, **the app cannot start**, and the ninth pass's milestone (chat working end-to-end) is not reproducible. An in-progress refactor was committed with two independent, stacked bugs: a new `packages/graph/nodes/` package collides with the existing `packages/graph/nodes.py` file (Python resolves in favor of the package, so the real `GraphNodes` class becomes unreachable), and two new `packages/rag/builders/__init__.py`/`packages/rag/pipelines/__init__.py` files were copy-pasted from an unrelated package and import a class that doesn't exist there. Either bug alone would block the app; together they make the tenth pass the sharpest single-pass regression yet (26.9% → 10.1% done), on par with the seventh pass's low point. One genuine fix landed underneath the outage — the LLM provider's missing API key from last pass is now correctly passed through — but it can't be demonstrated live until the app can start again. A real, substantial calculator tool also landed this pass, but crashes on every non-trivial expression due to a Python-version incompatibility. The pattern across the last several passes has been consistent: every regression has been the kind an import-smoke-test or a real pytest suite would catch in seconds, and none of them have been caught before landing. See [`docs/UNUSED_FILES.md`](./UNUSED_FILES.md), also updated this pass, for the growing list of copy-paste and duplicate-naming mistakes behind these regressions, plus a recurring pattern of accidental zip-archive commits now large enough to include entire package snapshots.
