...
- [ ] Upload a document with a distinctive, made-up product code → `202`, then ingestion completes off the request path, nonzero `chunk_count`. Check the API's startup log first to know which path to expect: `Job queue ready...` means it ran as a real arq job (worker logs `Queued ingestion finished`); `Could not connect to Redis...` means it fell back to the in-process path instead (`Background ingestion finished` in the API's own logs). Response's `data.file_id` should be a real UUID from the Upload Service, not a locally-generated placeholder — cross-check it against `SELECT file_id FROM documents WHERE id = '<document_id>';`, should match exactly.
- [ ] With Redis reachable, confirm the queue path specifically: start `worker` (`python -m packages.worker.main` or `docker compose up worker`) alongside `api`, upload a document, and confirm the **worker's** log (not the API's) shows `Queued ingestion finished` — proves the job actually crossed process boundaries via Redis, not just that ingestion happened somewhere.
