...
4. `packages/sdk/upload/` is done, not a cleanup target — fixed and wired in for real (see above). Delete `packages/sdk/notification/`, `packages/sdk/common/models.py` — or fix their import bugs first if either is meant to be used soon; as-is they're both unused *and* broken, same as `sdk/upload/` was before this pass.
