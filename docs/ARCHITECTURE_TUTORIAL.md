...
7. Schedules the actual work **off the request path**, then immediately returns `202 Accepted` with `DocumentUploadResponseSchema(status=PENDING, document_name=..., file_id=...)`. The caller doesn't wait for embedding work to finish. Two ways this can happen, chosen at request time by whether `container.queue.pool()` is a real connection:
   - **Real queue (the normal path)** — `pool.enqueue_job("ingest_document_job", ingestion_request, str(scratch_path))` hands it to the separate `worker` process (`packages/worker/jobs.py::ingest_document_job`, `packages/worker/main.py`), which runs the exact same `IngestionPipeline.ingest()` call inside its own `request_scoped_session(container)`, off a container built once per worker process at startup. On failure it re-raises after logging, so arq's own `max_tries=3` retries it — real durability, not just relocation.
   - **Fallback, only when Redis was unreachable at API startup** — `background_tasks.add_task(_ingest_in_background, container, ingestion_request, scratch_path)` runs the same pipeline in-process instead. `packages/api/lifespan.py` tries to connect a producer-side arq pool at startup and, on failure, logs a warning and leaves `container.queue.pool()` as `None` rather than blocking startup — the exact same non-fatal try/except idiom it already uses when the Postgres checkpointer connection fails (falling back to the in-memory `MemorySaver`, §4 above). Both ingestion paths open their **own fresh** `request_scoped_session(container)` rather than reusing the original request's — the original request's session may already be closed by the time either runs — and delete the scratch copy in a `finally` block regardless of whether ingestion succeeded, failed, or was skipped as a duplicate.
...
**A gap found via a real production incident, since fixed, two layers deep:** the extraction prompt originally had no instruction excluding the *model's own claims about its own capabilities* from being captured as durable facts — if the model answered a capability question incorrectly in one turn (e.g. wrongly claiming to be stateless), that wrong claim could get extracted and stored as a `FACT`, and because memory is recalled into every future conversation for that user, the error became self-reinforcing. `_EXTRACTION_PROMPT` now explicitly instructs the model never to capture the assistant's own self-referential claims, however confidently phrased — verified live with a real LLM call against a synthetic conversation containing exactly this kind of poisoned claim, correctly extracting only the legitimate user facts present. A second, defense-in-depth regex filter (`_is_self_referential()`, matched against the extracted content's phrasing, not just any mention of "the AI" in the source conversation) drops anything that still reads as a first-person AI self-description, in case a future model/prompt regression lets one slip past the instruction alone. See §13 for the incident and cleanup.
...
- **`IAMAuthSDK`** (`auth.py`) — `get_current_user(access_token)` calls the real IAM `/auth/me` endpoint with the bearer token, returning a `CurrentUser` Pydantic model. Also has `get_service_token()` (client-credentials grant), `refresh_token(refresh_token)`, and `introspect(session_id)` for session validation.
- **`IAMUsersSDK`** (`user.py`), **`IAMTenantsSDK`** (`tennets.py`) — user/tenant management endpoints, not exercised on the hot chat path.

**Routed through the real gateway (`https://gateway.easydev.in`), not the IAM service directly** — by request, later than the fixes below. This is a genuinely different routing scheme, not just a different host: the gateway routes by resource-group prefix (`/api/auth/*`, `/api/users/*`, `/api/tenants/*`, `/api/rbac/*`, `/api/sessions/*`, `/api/profile`, and `/api/iam/*` as the catch-all for anything without its own prefix — confirmed live this is specifically where `/health` lives) rather than IAM's own single `/api/v1/iam/*` prefix for everything. `packages/sdk/iam/endpoints.py`'s constants now bake in each route's real, live-confirmed gateway prefix directly. JWKS is confirmed **not** reachable through the gateway under any prefix — `IAMSettings.jwks_base_url` exists for it separately, unused since nothing calls JWKS yet. One real gateway-specific quirk found live: `/api/auth/refresh` returns a different error message through the gateway (`"Session expired. Please log in again."`) than calling IAM directly (`"Invalid refresh token"`) — every other route's error text matched exactly, suggesting the gateway layers its own session handling on top of refresh specifically.
...
**A real production incident this documentation was written just after, worth understanding as a case study in how this system can fail even when the code is correct:** a user reported the assistant confidently claiming to have no knowledge base and no persistent memory, describing RAG as "paste a document into this chat, I'll use it for this session only." Two genuine code bugs were behind the first layer of it (both now fixed, described in §4.13/§6.1): `PromptBuilder` misattributing retrieved context to the user, and no relevance floor on reranked results. But after fixing both, the *exact same wrong answer* kept recurring — because a single long-lived dev conversation had accumulated 117 messages of the model's own earlier, pre-fix wrong self-descriptions, and — more seriously — that user's long-term `memories` table had **63 stored rows**, most of which were the model's own hallucinated self-descriptions ("the AI is session-based and stateless," repeated with minor wording variations roughly ten times) captured as if they were durable facts, plus at least two fully fabricated product-brand associations. Because memory recall is tenant/user-scoped, not conversation-scoped, those poisoned rows were re-injected into **every** conversation for that user regardless of which one was used — the code fixes alone couldn't undo already-poisoned data. Both the conversation history and the memory rows were deleted for the affected tenant/user as a cleanup. **The underlying gap — nothing in `LLMMemoryExtractor`'s prompt (§7.2) excluded the model's own self-referential claims from being captured as facts — has since been fixed**, later the same day the roadmap first flagged it as still open: the extraction prompt now explicitly excludes the assistant's own self-descriptions, backed by a second, regex-based safety net that drops any that slip through anyway, both confirmed live against the exact kind of poisoned claim this incident produced. This remains the single most important thing to understand about why "the code is correct" and "the system behaves correctly" can diverge, independent of that fix: LLM-driven state (memory, conversation history) is itself an input to future behavior, and a wrong answer captured as state doesn't self-correct just because the code that produced it later gets fixed — the fix here closes the *recurrence* gap for every tenant going forward, not the fact that bad state can still slip through once and needs a human to notice and clean it up.
