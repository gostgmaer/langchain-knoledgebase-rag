...
Six nodes (`planner`, `load_memory`, `retrieve`, `tool`, `llm`, `extract_memory`), two conditional-edge decision points (`GraphRouter.route()` after `load_memory`, `GraphRouter.after_llm()` after `llm`), compiled with a checkpointer (`packages/infrastructure/container/graph.py`'s `MemorySaver`, wired as a `Singleton` so checkpoints genuinely persist across requests within the process's lifetime — see §13 for why that mattered).
...
4. Writes the uploaded bytes to `settings.storage.upload_directory / f"{uuid4()}_{file.filename}"` — a real file on disk, not held in memory.
5. Builds an `IngestionRequest` (`packages/knowledge/schemas.py`): `tenant_id`, `model_profile_id`, `knowledge_base_id`, `file` (the saved path), `document_name`, `chunking_strategy`.
6. `background_tasks.add_task(_ingest_in_background, container, ingestion_request)` — schedules the actual work **off the request path**, then immediately returns `202 Accepted` with `DocumentUploadResponseSchema(status=PENDING, document_name=...)`. The caller doesn't wait for embedding work to finish.
7. `_ingest_in_background()` opens its **own fresh** `request_scoped_session(container)` rather than reusing the original request's — a background task runs *after* the HTTP response is sent, by which point the original request's session may already be closed. This is the same reasoning documented in `packages/api/dependencies.py`.
...
- **Registered tools** (`packages/infrastructure/container/tools.py`'s `init_tool_manager()`): exactly these four — `get_weather`, `get_news`, `get_google_search`, `calculator`. Any other tool the model might claim to have is a hallucination, not a real capability.
...
**Present but confirmed unused** (see `docs/UNUSED_FILES.md` for the exhaustive, continuously-updated list): most of `packages/application/`'s mappers/validators/stub services, `packages/conversation/manager.py` + `store.py` + `memory_store.py`, `packages/sdk/upload/` + `packages/sdk/notification/` (both also currently broken on top of being unused), `packages/infrastructure/ai/factory.py` + `ai/registry.py`'s `LLMRegistry`, `packages/api/middleware/rate_limit.py` + `metrics.py`, six of ten API routers (`feedback`, `knowledge_bases`, `models`, `prompts`, `search`, `tools`).
