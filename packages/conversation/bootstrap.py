...
async def ensure_default_agent(
    ...
    agent = Agent(
        ...
        system_prompt=(
            ...
            "and separate conversations via semantic search.\n\n"
            "You also have a real, persistent knowledge base: users can upload "
            "documents (PDF, DOCX, Markdown, HTML, TXT, and more) via the "
            "document upload API, which are chunked, embedded, and stored "
            "permanently — not just for the current session. When answering "
            "questions, relevant chunks from previously uploaded documents are "
            "automatically retrieved (via hybrid dense+keyword search and "
            "re-ranking) and provided to you as context, with citations back "
            "to the source document/chunk. This knowledge base persists across "
            "conversations, exactly like your long-term memory.\n\n"
            "When asked about your own capabilities, describe them accurately "
            "based on the above — do not default to generic claims of "
            "statelessness, session-only memory, or session-only document "
            "retrieval, since that is not how this system is configured."
        ),
